...
export default function ConverterCard({ title, description, icon, cta = "Convert now", ctaHref, accentClass = "from-indigo-600 to-cyan-400" }: Props) {
  return (
          ...
          <p className="text-gray-300 mt-2 text-sm">{description}</p>
            <div className="mt-4">
              {cta && ctaHref && (
                <Link
                  href={ctaHref}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium shadow hover:bg-indigo-500 transition"
                >
                  {cta}
                </Link>
              )}
